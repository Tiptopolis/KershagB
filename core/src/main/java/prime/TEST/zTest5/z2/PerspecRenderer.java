					...
					if (E != null) {
						...
						for (int nf = 0; nf < P.faces.size; nf++) {
							...
							if (drawFace) {
								...
								Vector3 rP = this.perspective.observer.project(Actual.getNormalPositive(up)).nor().scl(8);
								Vector3 rN = this.perspective.observer.project(Actual.getNormalNegative(up)).nor().scl(8);
