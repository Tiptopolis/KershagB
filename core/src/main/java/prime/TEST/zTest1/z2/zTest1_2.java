...
public class zTest1_2 extends uApp {
	...
	public MapKernel Map; // quadTree this bitch
	Texture mapBase;
	float far = 0;
	...
	public void create() {
		...
		this.ExplorerKernel.c=this.Explorer.perspective.camera;
		this.mapSetup();
	...
	public void render() {
		...
		Array<rNode> tst = Map.getRegion(new Rect(0,0,10,10));
		Log(tst.size);
		Log("FROM: "+tst.get(0).toLog());
		Log("TO: "+tst.get(tst.size-1).toLog());
	}
	...
	public void mapSetup() {
		String path = "/assets/TST/TestInput";
		ArrayList<FileHandle> fGet = GdxFileUtils.getFilesFrom("." + path);
		HashMap<String, FileHandle> fMap = GdxFileUtils.mapFiles(fGet);
		this.mapBase = new Texture(fMap.get("LandWaterBool3"));
		this.Map = new MapKernel(this.Environment, this.mapBase);
		this.Map.connectObserver(this.ExplorerKernel);
	}
