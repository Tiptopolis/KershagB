...
public class LuciferParticle extends _Entity {
	...
	public static float largestZ = 0;
	public static float smallestZ = Float.MAX_VALUE;

	public LuciferParticle(ObserverKernel environment) {
		this.reference = environment.observer;
		this.Environment = environment;
		this.Environment.Members.add(this);
		this.memory = new OroborosList(1, 2, 1);
	...
	protected void draw() {
		...
		for (int i = m; i > 0; i--) {
			...
			float n = (1f / m) * 1f;
			// Log(n);
			Vector3 at = this.reference.project(this.memory.get(i).GetPosition().cpy());
			Vector3 v = this.memory.get(i).GetPosition();
			Vector3 s = this.memory.get(i).GetScale();
			s.x *= ar;
