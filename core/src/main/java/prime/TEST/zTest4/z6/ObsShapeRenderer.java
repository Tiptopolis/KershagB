...
public class ObsShapeRenderer {
	...
	public ObsShapeRenderer(ObserverKernel kernel) {
	...
	public void render() {
		...
		this.render(this.perspective.observer,0);
				...
				for (int j = 0; j < b; j++) {
					...
					if (E != null) {
						...
						for (int v = 0; v < G.vertices.size; v++) {
							...
							boolean F = this.perspective.observer.frustum.planes[0].testPoint(V.get) == Plane.PlaneSide.Front;
						...
						for(int l =0; l < G.lines.size; l++)
						{
							...
							boolean F1 = this.perspective.observer.frustum.planes[0].testPoint(L.from.get) == Plane.PlaneSide.Front;
							boolean S1 = (this.perspective.observer.frustum.sphereInFrustum(L.from.get, unit.len() * 3));
							boolean F2 = this.perspective.observer.frustum.planes[0].testPoint(L.to.get) == Plane.PlaneSide.Front;
