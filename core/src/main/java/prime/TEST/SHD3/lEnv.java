...
public class lEnv extends _Environment {
	...
	public void update(float deltaTime) {
		...
		// shader.setUniformf("u_camD",
		// this.observer.position.cpy().scl(this.observer.direction.cpy()));
		...
		shader.setUniformMatrix("u_invPrj", this.observer.invProjectionView);
		shader.setUniformMatrix("u_viewPrj", this.observer.projection);
	...
	public void bind(ShaderProgram s, boolean prj) {
		...
		for (int i = 0; i < shpCnt; i++) {
			...
			if (this.toBind.get(i) != null) {
				...
				float[] compound = VectorUtils.disassembleVect4(new Quaternion(p.x, p.y, p.z, z2));
				s.setUniform4fv("posAr[" + (i) + "]", compound, 0, compound.length);
			}
		}

		for (String S : s.getUniforms()) {
			Log(S);
		}
		Log("");
