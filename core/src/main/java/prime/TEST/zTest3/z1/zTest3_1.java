...
public class zTest3_1 extends uApp {
	...
	private void renderTrn(Transform T) {
		...
		Vector3 rightRot = pos.cpy().add(dir.cpy().crs(new Vector3(0, 0, 1)).nor().scl(unit.cpy()));
		...
		for (Transform t : Ts) {
			...
			Sketcher.setColor(F);
			Sketcher.Drawer.circle(tPos.x, tPos.y, (Ts.indexOf(t, true)));
		}
