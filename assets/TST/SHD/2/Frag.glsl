...
float sDTS(vec2 p)//uv lol
{
	...
	for(int i = 0; i <= shpCnt-1; i++)
	{
		...
		//float d =  distance(vec3(p,1),pA)-(posAr[i].w/u_resolution.y);	
		float d =  distance(vec3(p,1),pA)-(posAr[i].w/u_resolution.x);
		
		
		//float distance =  distance(vec3(p,1),pA);
	
		//float z = posAr[i].w/distance;
		//z  = (z  * 8) / 2; 
		//z*= 8;
